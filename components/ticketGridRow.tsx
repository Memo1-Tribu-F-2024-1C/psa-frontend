...
export default function TicketGridRow({ ticket }: { ticket: Ticket }) {
  ...
  const BorrarTicket = async (ticket: any) => {
    ...
    proyectosAxios.delete(`proyecto/${ticket.numeroTicket}`)
      .then(response => {
...
const editarDatos = (datos: any) => {
  proyectosAxios.put(`/tickets/${ticket.numeroTicket}`, datos)
    .then(response => {
  ...
  return (
    <tr key={`${ticket.numeroTicket}`}>
      ...
      <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
        <div className="text-m leading-5 text-gray-200">{ticket['fechaLimite']}</div>
      </td>

      <td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
        <div className=" text-m flex items-center text-gray-200">{ticket['cuitCliente']}</div>
      </td>
