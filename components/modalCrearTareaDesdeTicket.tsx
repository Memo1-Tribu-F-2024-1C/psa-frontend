...
const ModalCrearTareaDesdeTicket = ({ colaboradores, isOpen, onClose, children }: { colaboradores: Array<Colaborador>, isOpen: boolean; onClose: () => void; children: any }) => {
  const [nombre, setNombre] = useState("");
  const [descripcion, setDescripcion] = useState("");
  const [estado, setEstado] = useState("NUEVO");
  const [prioridad, setPrioridad] = useState("ALTA");
  const [tecnico, setTecnico] = useState('')
  const [proyectoUtilizado, setProyecto] = useState('');
  const [proyectos, setProyectos] = useState<Proyecto[]>([]);;
  const [tarea, setTarea] = useState({});
  
  const guardarTarea = (datos: any) => {
    ...
    <div className={styles.modalContainer} style={{ display: isOpen ? 'grid' : 'none' }}>
      <div className={styles.modalBody}>
        ...
        <div className='flex flex-row-reverse gap-10'>
          <button
            onClick={() => {
              guardarTarea({ nombre: nombre, descripcion: descripcion, estado: estado, prioridad: prioridad, colaboradorId: tecnico, proyectoId: proyectoUtilizado});
              onClose()
            }}
...
export default ModalCrearTareaDesdeTicket;