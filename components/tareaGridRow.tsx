...
export default function TareaGridRow({ tarea, idProyecto, colaboradores, mostrarAcciones }: { tarea: any; idProyecto?: any, colaboradores?: Array<Colaborador>, mostrarAcciones?: boolean }) {
  ...
  return (
    <tr key={`${tarea['id']}`}>
      ...
      {
        colaboradores != null ? (<td className="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
          {/* Botones Acciones */}

          <button
            onClick={() => setEditarTareaModal(true)}
            className="inline-flex items-center mt-1 mr-2 px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 text-m font-medium rounded-md">
          ...
          <button
            onClick={() => router.push({ pathname: `./tickets/${tarea['id']}` })}
            className="inline-flex items-center mt-1 mr-2 px-4 py-2 bg-blue-500 hover:bg-blue-700 text-white text-m font-medium rounded-md">
          ...
          <button
            ...
            className="inline-flex items-center mt-1 px-4 py-2 bg-red-600 hover:bg-red-700 text-white text-m font-medium rounded-md">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
